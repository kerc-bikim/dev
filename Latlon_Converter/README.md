...
--nearby 미터     주변 필지 목록도 출력 (지번이 기대와 다를 때 확인용)
...
연속지적도는 지적도(1:1200 등)와 임야도(1:6000)를 각각 이어 붙여 만들기 때문에, **한 점에
토지대장 필지와 임야대장 필지가 겹쳐 등록되어 있는 경우**가 있습니다. 예를 들어 같은 좌표에
`가을리 853`(대)과 `가을리 산 306`(임야)이 함께 걸립니다.

이 도구는 겹친 필지를 모두 받아 본 뒤 이렇게 고릅니다.

1. 입력한 점을 **실제 경계 안에 품는** 필지만 남깁니다.
2. 그중 **가장 작은** 필지를 고릅니다. 겹쳐 있을 때 더 촘촘히 등록된 쪽(대개 지적도 필지)입니다.
3. 고르지 않은 나머지는 `비고`와 JSON의 `alternatives`에 남겨 확인할 수 있게 합니다.

그래도 기대한 지번과 다르면 주변 필지를 함께 확인해 보세요.

```bash
../.venv/bin/python -m latlon_converter point --lat 37.968352 --lon 124.645289 --nearby 50
```

점이 어느 필지 경계에도 들어가지 않는다고 나오면 다음을 의심할 수 있습니다.

- **연속지적도의 위치 오차**: 도해지적을 이어 붙인 자료라 실제 경계와 수 m 어긋날 수 있습니다.
- **측지계 차이**: 좌표 출처가 동경측지계(구 좌표계)라면 WGS84와 수백 m 차이가 납니다. 이 도구는
  입력을 WGS84로 봅니다.
...
인증키는 앞 4자리와 길이만 보여 줍니다. 오류 메시지와 디버그 로그에 찍히는 요청 URL에서도
`key=<가려짐>`으로 바뀌므로, 실행 결과를 그대로 공유해도 키가 새지 않습니다.
...
1. `req/data` 연속지적도(`LP_PA_CBND_BUBUN`)를 좌표로 질의 — PNU, 지번, 주소, 공시지가를 한 번에 받습니다.
   겹쳐 등록된 필지가 있을 수 있어 경계 도형과 함께 여러 건을 받아 고릅니다(위 "지번이 기대와 다를 때" 참고).
