...
# -*- coding: utf-8 -*-
"""
QSCD20 UDP 수신 GUI v3 (PyQt5 + pyqtgraph)
...
class StationBuffer:
    ...
    def _prune(self) -> None:
        if len(self.buckets) <= CHART_WINDOW_SEC + 30:
            return
        max_sec = max(self.buckets.keys())
        cutoff = max_sec - CHART_WINDOW_SEC - 10
        for s in list(self.buckets.keys()):
            if s < cutoff:
                del self.buckets[s]

    def grid(self, field: str) -> Tuple[np.ndarray, np.ndarray]:
        return build_second_grid(self.buckets, field)
...
class QscdChartDashboard(QtWidgets.QWidget):
    def __init__(self, parent: Optional[QtWidgets.QWidget] = None) -> None:
        ...
        self._cached_buckets_id: Optional[int] = None
    ...
    def _make_plot(
        ...
        plot = gl.addPlot(row=row, col=0, title=title, axisItems={"bottom": axis})
    ...
    def _ensure_grid_cache(self, buckets: Dict[int, Dict[str, float]]) -> None:
        bid = id(buckets)
        if bid == self._cached_buckets_id and self._grid_cache:
            return
        self._cached_buckets_id = bid
        self._grid_cache.clear()
        fields = {"diff"}
        fields.update(SERIES_IDX.keys())
        for f in fields:
            self._grid_cache[f] = build_second_grid(buckets, f)

    def update_from_buckets(self, buckets: Dict[int, Dict[str, float]]) -> None:
        ...
        self._ensure_grid_cache(buckets)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("QSCD20 UDP Receiver v3")
    ...
    def _on_panel_check_changed(self) -> None:
        self._live_charts.set_active_panels(self._get_checked_panels())
        self._schedule_chart_refresh(self._selected_station)
    ...
    def _schedule_chart_refresh(self, station: str) -> None:
        self._chart_dirty_station = station
    ...
    def _on_packet(self, pmyqscd: bytes, myqscd: Tuple[Any, ...]) -> None:
        ...
        if st not in self._station_bufs:
            self._station_bufs[st] = StationBuffer()
        ...
        self._update_last_recv_label()
        self._schedule_chart_refresh(st)
