...
# -*- coding: utf-8 -*-
"""
QSCD20 UDP 수신 GUI v3.2 (PyQt5 + pyqtgraph)
...
def log_qscd_packet_summary(
    ...
    logger.info(
        "[{}] Q={} {} | data={} diff={:.3f}s | Zmax={:.6g} Hpga={:.6g}".format(
            station,
            describe_quality_flag(byte_to_u8(myqscd[1])),
            format_station_code_line(myqscd).split("|")[0].strip(),
...
class ChartHoverTooltip(QtCore.QObject):
    ...
    def __init__(
        ...
        self._text = pg.TextItem(anchor=(0, 1), border=pg.mkPen(color=(100, 149, 237), width=1))
    ...
    def _on_mouse(self, evt: Tuple[Any, ...]) -> None:
        ...
        if len(lines) <= 1:
            ...
            return
        self._text.setText("\n".join(lines))
        self._text.setPos(ts, mouse_point.y())
        self._text.show()
...
class QscdChartDashboard(QtWidgets.QWidget):
    ...
    def _make_plot(
        ...
        y_label: str,
        row: int,
    ) -> Tuple[Dict[str, pg.PlotDataItem], ChartHoverTooltip]:
    ...
    def _rebuild(self) -> None:
        ...
        for pid in self._active_panels:
            ...
            curves, hover = self._make_plot(gl, title, curves_def, y_label, 0)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("QSCD20 UDP Receiver v3.2")
    ...
    def _build_ui(self) -> None:
        ...
        tw_row.addWidget(
            QtWidgets.QLabel("  · 드래그: 줌인  · 더블클릭: 줌아웃  · 마우스 오버: 값 팝업")
        )
        tw_row.addStretch()
    ...
    def _on_file_panel_check_changed(self) -> None:
        ...
        if self._file_records:
            self._file_charts.set_static_from_records(
                self._file_records, recv_diffs=self._file_recv_diffs
            )
    ...
    def _drain_packet_queue(self) -> None:
        ...
        if last_selected is not None:
            ...
            self._lbl_live_station.setText(format_station_code_line(last_selected))
            self._update_last_recv_label()
    ...
    def _open_bin_file(self) -> None:
        ...
        self._lbl_file_info.setText(
            f"{path}\n레코드 수: {len(recs)}\n{diff_note}\n"
            f"시작(KST, data time): {t0}\n종료: {t1}\n"
            f"Quality: {q0}\nStation: {st0}"
        )
        self._file_charts.set_window_sec(int(self._combo_file_time_window.currentData()))
        self._file_charts.set_active_panels(self._get_file_checked_panels())
        self._file_charts.set_static_from_records(recs, recv_diffs=self._file_recv_diffs)
