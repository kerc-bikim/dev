...
**현재 버전: v3.8** ([recvQSCD20_gui.py](recvQSCD20_gui.py), [VERSION](VERSION))
...
| 로그 / bin | `{prefix}.QSCD.log` → `logs/`, `{prefix}.QSCD20.bin` → `bin/` (QCDX) |
...
| UI | Live/File 탭 + **우측 수신 패널**; 로그·기본 설정 메뉴 팝업 |
...
| 파일 | 역할 |
|------|------|
| [recvQSCD20_gui.py](recvQSCD20_gui.py) | GUI **v3.8** |
