...
class QscdChartDashboard(QtWidgets.QWidget):
    ...
    def _make_plot(
        ...
        y_label: str,
        row: int,
    ) -> Tuple[Dict[str, pg.PlotDataItem], ChartHoverTooltip, pg.PlotItem]:
        axis = KSTTimeAxisItem(orientation="bottom")
        vb = ChartViewBox()
        plot = gl.addPlot(row=row, col=0, title="", viewBox=vb, axisItems={"bottom": axis})
    ...
    def _rebuild(self) -> None:
        ...
        for pid in self._active_panels:
            ...
            curves, hover, plot = self._make_plot(gl, curves_def, y_label, 0)
