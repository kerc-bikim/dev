...
- -B <i>bytes</i>
  Re-pack all output records to the specified miniSEED record/block size.  The value must be a power of 2 between 128 and 131072 bytes (for example 512, 1024, 4096).  Each input record is unpacked and written as one or more records of this length.  When this option is not specified the original record length is preserved.  Records that cannot be unpacked (unsupported encodings) are written unchanged with a warning.
