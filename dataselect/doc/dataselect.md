...
- -B <i>bytes</i>
  Re-pack all output records to the specified miniSEED record/block size.  The value must be a power of 2 between 128 and 131072 bytes (for example 512, 1024, 4096).  Sample times and sample values are unchanged: the output time-series is identical to the input, only the record/block framing changes.  Continuous traces of the same channel are buffered until a record fills, then written.  A short record is written at a time gap, when the stream changes, or at the end of output.  Overlapping records are not merged.  For miniSEED 2 the records are exactly <i>bytes</i> long; for miniSEED 3 <i>bytes</i> is the maximum record length.  If a record cannot be re-packed to the requested size (unsupported encoding, or headers larger than the block size) the program exits with an error rather than writing the original record.  miniSEED 2 sequence numbers are rewritten per channel in each output file starting at 1; a new archive file starts again at 1.  Sequences are left unchanged when <b>-B</b> is omitted.
