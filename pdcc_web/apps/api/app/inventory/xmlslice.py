...
from .xmlbuild import InventoryError
...
def parse_nslc(nslc: str) -> tuple[str, str]:
    text = (nslc or "").strip()
    if not text:
        raise InventoryError("채널이 필요합니다", 400, "E_EXPORT")
    if "." in text:
...
def slice_stationxml(
    ...
    *,
    ...
    if not (xml or "").strip():
        raise InventoryError("StationXML이 없습니다", 400, "E_EXPORT")
    root = parse_root(xml)
    ...
    for net in list(root):
        ...
        if kept_sta == 0:
            ...
            kept_net += 1
    if kept_net == 0:
        raise InventoryError("내보낼 관측소·채널이 없습니다", 404, "E_EXPORT")
    return dumps(root)


def channel_count(xml: str) -> int:
    root = parse_root(xml)
    return sum(1 for node in root.iter() if local(node.tag) == "Channel")


def station_codes(xml: str) -> list[str]:
    root = parse_root(xml)
    codes: list[str] = []
...
def iter_channels(xml: str) -> list[etree._Element]:
    root = parse_root(xml)
    return [node for node in root.iter() if local(node.tag) == "Channel"]
