...
pg_restore --list "$BACKUP_DIR"/pdcc-*.dump >/dev/null`}</code></pre>
      ...
      <section id="admin-chapter-8" className="manual-chapter">
        ...
        <p>
          ...
          저장 여부를 확인합니다. 대시보드의 장기 잠금에서 관측소 경로와 편집자를 확인한 뒤, 사유를
          넣고 <strong>강제 해제</strong>합니다. 기본 TTL은 5분이므로 10분 이상 목록이 비어 있어도
          <strong>현재 잠금</strong>에서 활성 잠금을 해제합니다. 강제 해제 후 해당 사용자는 화면을 새로 열어 최신 버전을 확인해야 하며, 관리자 감사 로그에
          <strong>잠금 강제 해제</strong>와 사유가 남았는지 확인합니다.
        </p>
      </section>
