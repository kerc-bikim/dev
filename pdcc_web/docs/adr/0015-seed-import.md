...
2. 변환은 `SEED_CONVERTER_JAR` 또는 `infra/jars/stationxml-seed-converter.jar` 가 있으면 공식 converter, 없으면 ObsPy `read_inventory(format="SEED")` → StationXML 1.2. 파형 MiniSEED·full SEED·zip·RESP는 거부한다.
