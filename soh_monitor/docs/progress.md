...
| M5.3 | 기록계 CRUD + Credential 분리 | 완료 | `credentialReference`(env:/file:) 만 저장. 평문 비밀번호 거절. `dataSourceUri` 는 PUT `null` 로 비우고 http/https/fdsnws/seedlink 만 허용. 등록 뒤 호스트·인증 참조 수정도 같은 규칙 |
...
| M6.7 | 연결 시험 진행 표시 | 완료 | 경과 초와 취소(`AbortController`). 등록 후에도 관측소 상세에서 시험. SOH 미리보기도 같음 |
...
| M6.10 | 관측소 상세 | 완료 | 분류 탭·현재값·수집 이력·Grafana Deep Link. 설정에서 접속·URI·수집·Override·센서·외부 SOH(ADMIN) |
