...
현재 상태: **M0(저장소 골격)·M1(계약 확정)·M2(Centaur CTR Adapter) 완료.** 작업별 실제 진행 상태는 [`docs/progress.md`](docs/progress.md) 에 있다. 이 문서는 계획 원본이며, 구현이 계획과 달라진 부분은 `progress.md` 에 기록한다.
